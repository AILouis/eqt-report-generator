...
def extract_citations_and_clean(
    ...
    # 2. Split-line variant: [Title] on one line, (https://url) on the very next line.
    ...
    # 3. Safety-net: strip the entire SOURCES / REFERENCES block if present.
    ...
    # 4. Bracketed citations without a URL: [Title - domain.com]
    cleaned = re.sub(
    ...
    # 5. Remove ALL remaining [text] brackets unconditionally.
    #    Steps 1/2 already collected every legitimate [text](url) pair, so anything
    ...
    # 6. Unclosed brackets: text cut off at token limit, e.g. "[Title that never closed
    cleaned = re.sub(r"\[[^\]\n]{0,300}$", "", cleaned, flags=re.MULTILINE)

    # 7. Bare URLs in parentheses — handles all forms:
    ...
    # 8. Remove leftover (, YYYY-MM-DD) shells — remnants of ([Link](url), Date) inline citations
    #    after the markdown link portion was stripped in step 1.
    cleaned = re.sub(r"\(\s*,\s*\d{4}-\d{2}-\d{2}\s*\)", "", cleaned)

    # 9. Remove inline author-date citations: (Source Name, YYYY-MM-DD)
    ...
    # 10. Any remaining bare https:// or http:// URLs
    cleaned = re.sub(r"https?://\S*", "", cleaned)

    # 11. Clean up whitespace artifacts left after removals
    ...
    # 12. Remove lines that now consist entirely of stray punctuation / parentheses.
    ...
    # 13. Drop lines that are now empty or only contain bare bullet markers
    lines_out = []
    ...
    # 14. Collapse 3+ consecutive blank lines into a single blank line
    out = "\n".join(lines_out).strip()
    out = re.sub(r"\n{3,}", "\n\n", out)

    # 15. Strip CJK characters from body text — report content must be English-only.
